...
namespace msgpack {
    ...
    typename std::enable_if<type_traits::is_basic_json<T>::value &&
                            type_traits::is_byte_sequence<Source>::value,T>::type 
    decode_msgpack(std::allocator_arg_t, const Allocator& alloc,
                   const Source& v, 
                   const msgpack_decode_options& options = msgpack_decode_options())
    {
        json_decoder<T> decoder(alloc);
        auto adaptor = make_json_visitor_adaptor<json_visitor>(decoder);
        basic_msgpack_reader<jsoncons::bytes_source,Allocator> reader(v, adaptor, options, alloc);
        reader.read();
        if (!decoder.is_valid())
        {
            JSONCONS_THROW(ser_error(conv_errc::conversion_failed, reader.line(), reader.column()));
    ...
    decode_msgpack(std::allocator_arg_t, const Allocator& alloc,
                   ...
                   const msgpack_decode_options& options = msgpack_decode_options())
    {
        basic_msgpack_cursor<bytes_source,Allocator> cursor(v, options, alloc);
                   ...
                   const msgpack_decode_options& options = msgpack_decode_options())
    {
        json_decoder<T> decoder(alloc);
        auto adaptor = make_json_visitor_adaptor<json_visitor>(decoder);
        basic_msgpack_reader<jsoncons::binary_stream_source,Allocator> reader(is, adaptor, options, alloc);
        reader.read();
        if (!decoder.is_valid())
        {
            JSONCONS_THROW(ser_error(conv_errc::conversion_failed, reader.line(), reader.column()));
    ...
    decode_msgpack(std::allocator_arg_t, const Allocator& alloc,
                   ...
                   const msgpack_decode_options& options = msgpack_decode_options())
    {
        basic_msgpack_cursor<binary_stream_source,Allocator> cursor(is, options, alloc);
        json_decoder<basic_json<char,sorted_policy,Allocator>> decoder(alloc);
