...
namespace bson {
    ...
    typename std::enable_if<type_traits::is_basic_json<T>::value &&
                            type_traits::is_byte_sequence<Source>::value,T>::type 
    decode_bson(std::allocator_arg_t, const Allocator& alloc,
                const Source& v, 
                const bson_decode_options& options = bson_decode_options())
    {
        json_decoder<T> decoder(alloc);
        auto adaptor = make_json_visitor_adaptor<json_visitor>(decoder);
        basic_bson_reader<jsoncons::bytes_source,Allocator> reader(v, adaptor, options, alloc);
        reader.read();
        if (!decoder.is_valid())
        {
            JSONCONS_THROW(ser_error(conv_errc::conversion_failed, reader.line(), reader.column()));
    ...
    decode_bson(std::allocator_arg_t, const Allocator& alloc,
                ...
                const bson_decode_options& options = bson_decode_options())
    {
        basic_bson_cursor<bytes_source,Allocator> cursor(v, options, alloc);
        json_decoder<basic_json<char,sorted_policy,Allocator>> decoder(alloc, alloc);
                ...
                const bson_decode_options& options = bson_decode_options())
    {
        json_decoder<T> decoder(alloc);
        auto adaptor = make_json_visitor_adaptor<json_visitor>(decoder);
        basic_bson_reader<jsoncons::binary_stream_source,Allocator> reader(is, adaptor, options, alloc);
        reader.read();
        if (!decoder.is_valid())
        {
            JSONCONS_THROW(ser_error(conv_errc::conversion_failed, reader.line(), reader.column()));
    ...
    decode_bson(std::allocator_arg_t, const Allocator& alloc,
                ...
                const bson_decode_options& options = bson_decode_options())
    {
        basic_bson_cursor<binary_stream_source,Allocator> cursor(is, options, alloc);
        json_decoder<basic_json<char,sorted_policy,Allocator>> decoder(alloc, alloc);
